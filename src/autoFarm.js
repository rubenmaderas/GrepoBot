class AutoFarm extends ModernUtil {
        ...
        for (let town_id of polis_list) {
            ...
            for (let farm_town of farm_town_models) {
                ...
                for (let relation of player_relation_models) {
                    ...
                    let option = 1;
                    if (this.timing === 600_000) option = 2; // 10 min
                    else if (this.timing === 1_200_000) option = 3; // 20 min
                    else if (this.timing === 2_400_000) option = 4; // 40 min
    ...
    main = async () => {
        ...
        if (this.timer < 1) {
            ...
            // Set the new timer (blindado a 5 minutos mínimo)
            const rand = Math.floor(Math.random() * 20_000) + 10_000;
            this.timer = Math.max(this.timing || 300000, 300000) + rand;
        }
